...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.tiles = {(x, y): dirt_amount for y in range(height) for x in range(width)}
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if self.tiles[position] - capacity >= 0:
            self.tiles[position] -= capacity
        else:
            self.tiles[position] = 0
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        # raise NotImplementedError
        clean_tiles = len([x for x in self.tiles if self.is_tile_cleaned(x[0], x[1])])
        return clean_tiles
    ...
    def is_position_in_room(self, pos):
        ...
        if (
            pos.get_x() <= 0
            or pos.get_x() >= self.width
            or pos.get_y() <= 0
            or pos.get_y() >= self.height
        ):
            return False
        else:
            return True
...
class EmptyRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        """
        # raise NotImplementedError
        return len([x for x in self.tiles])
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        """
        # raise NotImplementedError
        return len([x for x in self.tiles if x not in self.furniture_tiles])
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        # raise NotImplementedError
        position = self.get_robot_position().get_new_position(
            self.get_robot_direction(), self.speed
        )
        ...
        if self.gets_faulty():
            ...
            if self.room.is_position_valid(position):
                ...
                self.room.clean_tile_at_position(
                    self.get_robot_position(), self.capacity
                )
            else:
                self.set_robot_direction(random.random() * 360)
...
#       of a 20x20 room?
#   The difference in performance is small. Overall, they have near identical trend lines.
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions
#       10x30, 20x15, 25x12, and 50x6?
#   The difference in performance for cleaning larger areas is far more apparent.
#   The standard robot is more efficient than the faulty robot by, at minimum, a factor of 2.
#
...
def show_plot_room_shape(title, x_label, y_label):
    ...
    for width in [10, 20, 25, 50]:
        ...
        times2.append(
            run_simulation(2, 1.0, 1, width, int(height), 3, 0.8, 200, FaultyRobot)
        )
